document.addEventListener("DOMContentLoaded", function() {
    ...
    function displayChosenResults() {
        ...
        // あなたの趣味に近いもの
        const hobbies = scoreTable.filter(row => row.sy1 === 'a');
        ...
        // あなたの趣味に近い好きなことの要素
        const likeFactors = scoreTable.filter(row => row.sy1 === 'i');
        ...
        const importantFactors = scoreTable.filter(row => row.sy1 === 'u');
        ...
        // 特に得意なこと
        const skills = scoreTable.filter(row => row.sy1 === 'e').sort((a, b) => b.score - a.score);
        const topSkills = [];
        let topScore = null;

        skills.forEach(skill => {
            if (topScore === null || skill.score === topScore) {
                topSkills.push(skill);
                topScore = skill.score;
            } else if (topSkills.length === 0 || topSkills.length === 1) {
                topSkills.push(skill);
            }
        });
